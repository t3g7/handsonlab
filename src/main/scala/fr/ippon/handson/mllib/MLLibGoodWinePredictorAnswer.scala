 ...
 * @author dmartin
 */
object MLLibGoodWinePredictorAnswer {
  ...
  def partieA(trainData: RDD[LabeledPoint], testData: RDD[LabeledPoint]) {
    ...
    val modelDT = DecisionTree.trainClassifier(trainData, numClassesDT, categoricalFeaturesInfoDT,
  ...
  def partieB(trainData: RDD[LabeledPoint], testData: RDD[LabeledPoint]) {
    ...
    Seq(10, 100, 200, 300).map { trees =>
      Seq(5, 10, 15, 20).map { depth =>
        val model = RandomForest.trainClassifier(trainData, numClassesRF, categoricalFeaturesInfoRF,
          trees, featureSubsetStrategyRF, impurityRF, depth, maxBinsRF)
        val predAndlabel = testData.map { x =>
          (model.predict(x.features), x.label)
        }
        evaluateAndPrint(predAndlabel, "Random Forest with trees=" + trees + " and depth=" + depth)
      }
    }
  ...
  def partieC(trainData: RDD[LabeledPoint], testData: RDD[LabeledPoint]) {
    ...
    val iters = 100
    val modelSVM = SVMWithSGD.train(trainData, iters)
    ...
    // evaluer et afficher
    evaluateAndPrint(predictionAndLabelSVM, "SVM")
    ...
    val modelRF = RandomForest.trainClassifier(trainData, numClassesRF, categoricalFeaturesInfoRF,
    ...
    evaluateAndPrint(predictionAndLabelRF, "Random Forest")
    ...
    // evaluation
    evaluateAndPrint(predictionAndLabelGBT, "Gradient Boosted Trees")

    // NaiveBayes : Utiliser l'objet NaiveBayes pour entrainer un modele :
    val modelNB = NaiveBayes.train(trainData)
    val predictionAndLabelNB = testData.map { point =>
      (modelNB.predict(point.features), point.label)
    }
    evaluateAndPrint(predictionAndLabelNB, "Naive Bayes")

  }

  /*
   * Si chargement d'un CSV, suppression de la première ligne via cette fonction
   */
  def skipFirstLine(raw: RDD[String]) {
    val header = raw.first()
    raw.filter { x => x != header }
  }
